// mute is a small utility for ALSA mixer volume
...
NAN_METHOD(setMute)
{
	...
	val = Nan::To<bool>(info[2]).FromJust();
	...
	if (err = snd_mixer_selem_has_playback_switch(elem) < 0)
		error_close_exit("Mute set error: %s\n", err, h_mixer);
...
NAN_METHOD(toggleMute)
{
	...
	snd_mixer_selem_get_playback_switch(elem, CHANNEL, &currentState);
	nextState = !currentState;

	snd_mixer_selem_set_playback_switch_all(elem, nextState);

	snd_mixer_close(h_mixer);

	info.GetReturnValue().Set((bool)!nextState);
}
